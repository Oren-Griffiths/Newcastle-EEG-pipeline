...
function X6_ArtifactRejection(DataConfig,SUB, imageType)
...
clearvars -except DataConfig SUB imageType;
    ...
    for i = 1:length(SUB)
        ...
        pop_saveset(EEG, 'filename', [SUB{i} '_ds_addChans_cleanline_asr_lp_refs_event_weighted_epoch_bl_ar.set'], 'filepath', Subject_Path);   
%              'savenew', [Subject_Path filesep SUB{i} '_ds_addChans_cleanline_asr_lp_refs_event_weighted_epoch_bl_ar.set'], 'gui', 'off');
...
%         % Oren wrote this bit.
%         data = EEG.data(:, :,:);
%         chanlocs = EEG.chanlocs(1:size(EEG.data,1));
